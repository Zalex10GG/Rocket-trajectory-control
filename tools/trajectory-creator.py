...
def build_passive_rocket(case_data):
    ...
    motor = GenericMotor(
        ...
        propellant_initial_mass=require_key(
            m_params, "propellant_initial_mass_kg", "motor"
        ),
        nozzle_radius=require_key(m_params, "nozzle_radius_m", "motor"),
        dry_mass=require_key(m_params, "dry_mass_kg", "motor"),
        center_of_dry_mass_position=require_key(
            m_params, "center_of_dry_mass_position_m", "motor"
        ),
        ...
        coordinate_system_orientation=require_key(
            m_params, "coordinate_system_orientation", "motor"
        ),
    ...
    rocket_kwargs = {
        ...
        "coordinate_system_orientation": require_key(
            body, "coordinate_system_orientation", "body"
        ),
    }
...
def main():
    ...
    if VERBOSE:
        print(
            f"Running simulation (inclination={INCLINATION_DEG}, heading={HEADING_DEG})..."
        )
    ...
    if len(header) != 7:
        print(
            f"CRITICAL ERROR: Reference header in {REFERENCE_HEADER_CSV_PATH} has {len(header)} columns, expected 7."
        )
    ...
    print(
        f"Final position (local ENU): ({final_row.iloc[1]:.2f}, {final_row.iloc[2]:.2f}, {final_row.iloc[3]:.2f})"
    )
